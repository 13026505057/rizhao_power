            ...
            <el-form :model="ruleForm"  ref="ruleForm" label-width="100px" class="demo-ruleForm">
              ...
              <el-form-item label="展示图上传" prop="desc">
                <el-upload
                  class="upload-demo"
                  name="file"
                  action="http://119.29.93.138:8080/stop/uploadPic"
                  :headers="myHeaders"
...
<script>
    ...
    export default {
        ...
        methods: {
            onload(){
                const self = this;
                var art_id = localStorage.getItem('articlexiugai_id');
                var arr = self.imgArr;
                console.log(arr);
                var string = arr.join(',');
                console.log(string);
                ...
                self.$axios({
                    ...
                    if(data.data.code==0){
                      self.content = data.data.data.art_content;
                      self.ruleForm.name = data.data.data.art_name;
                      self.ruleForm.desc = data.data.data.art_desc;
                      self.auth = data.data.data.art_auth;
                      self.imgArr = data.data.data.art_cover.split(',')
                    }else{
