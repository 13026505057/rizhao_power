<template>
    <div v-loading.fullscreen.lock="loadingAll" v-infinite-scroll="loadMore" infinite-scroll-disabled="busy" infinite-scroll-distance="2" class="FirBox">
        <div class="block">
            <el-select clearable v-model="good_name" style="height:40px;" remote reserve-keyword :remote-method="remoteMethod" :loading="loading" filterable placeholder="物品名称">
                <el-option style="height:40px;" v-for="item,index in goodsNameList" :key="index" :label="item.label" :value="item.label">
                </el-option>
            </el-select>
            <el-input style="width:200px;margin-left:20px;" placeholder="物品编码" v-model="good_number" clearable>
            </el-input>
        ...
        <div class="" id="box">
            <ul>
                <li v-for="v in json.list">
                    ...
                    <h5 style="width:200px;">库存地点:{{v.huowei}}</h5>
                    ...
                    <h5>库存:
                        <span style="color:red;margin-left:10px;">{{v.quantity}}</span>
                    </h5>
                    <div>
                        <el-row>
                            <el-col :span="12">
                                <div class="grid-content">
                                    <el-button size="mini" @click="applyNow(v)" type="warning">物资领用</el-button>
                                </div>
                            </el-col>
                            <el-col :span="12">
                                <div class="grid-content">
  ...
  methods: {
    ...
    addApplyGoods(res) {
      ...
      if (self.applyForm.quantity == "") {
        ...
        params.append("in_store_id", "");
        params.append("stock_id", self.applyForm.stock_id);
        params.append("quantity", self.applyForm.quantity);
        params.append("apply_type", "out");
    ...
    remoteMethod(query) {
      if (query !== "") {
        ...
        setTimeout(() => {
          this.loading = false;
          this.goodsNameList = this.list.filter(item => {
    ...
    applyNow(res) {
      ...
      this.applyForm.stock_id = res.stock_id;
          ...
          if (data.data.code == 0) {
            for (var i = 0; i < data.data.data.pageInfo.list.length; i++) {
              if (
                ...
                data.data.data.pageInfo.list[i].good_banner_img == null ||
                data.data.data.pageInfo.list[i].good_banner_img == "null"
              ) {
                data.data.data.pageInfo.list[i].good_banner_img = [];
              } else {
                data.data.data.pageInfo.list[
                  i
                ].good_banner_img = data.data.data.pageInfo.list[
                  i
                ].good_banner_img.split(",");
              }
            }
            ...
            self.json.list = self.json.list.concat(
              data.data.data.pageInfo.list
            );
...
.FirBox::-webkit-scrollbar {
  /*滚动条整体样式*/
  width: 4px; /*高宽分别对应横竖滚动条的尺寸*/
  height: 4px;
}
.FirBox::-webkit-scrollbar-thumb {
  /*滚动条里面小方块*/
  border-radius: 5px;
  -webkit-box-shadow: inset 0 0 5px rgba(0, 0, 0, 0.2);
  background: rgba(0, 0, 0, 0.2);
}
.FirBox::-webkit-scrollbar-track {
  /*滚动条里面轨道*/
  -webkit-box-shadow: inset 0 0 5px rgba(0, 0, 0, 0.2);
  border-radius: 0;
