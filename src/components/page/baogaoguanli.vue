...
<!--         <div >
        ...
        <div id="container" style="width: 100%;height: 110%;  float: right;">
          <div class="numData">
            ...
            <div class="tableTitle">
              <el-button type="primary" @click="showAddNew">新增报告</el-button>
            </div>
          </div>
            <el-dialog title="新增报告" :visible.sync="dialogFormVisible">
              <el-form :model="form">
                <el-form-item label="报告名称:" label-width="120px">
                  <el-input v-model="form.report_name" autocomplete="off" style="width:400px;"></el-input>
                </el-form-item>
                <el-form-item label="报告描述:" label-width="120px">
                  <el-input v-model="form.report_desc" autocomplete="off" style="width:400px;"></el-input>
                </el-form-item>
                <el-form-item label="报告类型:" label-width="120px">
                  <el-input v-model="form.report_desc" autocomplete="off" style="width:400px;"></el-input>
                </el-form-item>
              </el-form>
            ...
            <el-dialog title="修改资料" :visible.sync="dialogFormVisible2">
              <el-form :model="form">
                ...
                <el-form-item v-if="form.report_desc=='尿检报告'?true:false" label="公章上传:" label-width="120px">
                  <el-upload
                    class="upload-demo"
                    drag
                    :on-success = 'uploadSuccess'
                    :headers="myHeaders"
                    :action="gongzhangUpload"
                    multiple>
                    <i class="el-icon-upload"></i>
                    <div class="el-upload__text">将文件拖到此处，或<em>点击上传</em></div>
                    <div class="el-upload__tip" slot="tip">只能上传jpg文件，且不超过5Mb，建议宽高比1:1</div>
                  </el-upload>
                </el-form-item>
              </el-form>
              <div slot="footer" class="dialog-footer">
...
<script>
  ...
  export default {
      ...
      mounted() {
          ...
          this.gongzhangUpload = this.$axios.defaults.baseURL+'/stop/uploadOrgImg';
      ...
      methods: {
          ...
          upData(data){
                ...
                var params = new URLSearchParams();
                var token = localStorage.getItem('auth');
                params.append('report_name',self.form.report_name);    
                params.append('report_id',self.form.report_id);
                params.append('report_desc',self.form.report_desc);
                self.$axios({
                    method: 'post',
                    url: '/stop/updateReport',
                    data: params,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
                 }).then(function(data){
                    if(data.data.code==0){
