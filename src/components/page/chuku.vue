        ...
        <div id="container" style="width: 100%;height: 110%; margin-top: 20px;">
          ...
          <div class="tableList">
            <el-tabs type="border-card" @tab-click="tableClick" v-model="tabPosition">
              <el-tab-pane label="待出库" name="need">
                <el-table
                  :data="goodsDetail"
                    ...
                    <template slot-scope="props">
                        <div class="stepLine">
                          <el-steps  align-center>
                            <el-step :status="item.apply_approve_result==1?'success':'finish'" v-for="item,index in props.row.userApplyApprove" :title="item.apply_approve_result==1?'已审批':'待审批'" :description="item.apply_approve_dept_name+item.apply_approve_user_name+item.apply_approve_msg"></el-step>
                          </el-steps>
                        </div>
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column
                    ...
                    <template slot-scope="props">
                      ...
                      <el-button size="mini" type="warning" style="" @click.stop="approve(props.row)">确认出库</el-button>
                      <el-button size="mini" type="warning" style="" @click.stop="notapprove(props.row)">暂不出库</el-button>
                    </template>
                  </el-table-column>
              ...
              <el-tab-pane label="出库记录" name="history">
                <el-table
                  ...
                  <el-table-column
                    ...
                    >
                    <template slot-scope="props">
                      <span>{{props.row.userApplyHead.user_apply_status==0?'驳回':''}}{{props.row.userApplyHead.user_apply_status==1?'通过':''}}{{props.row.userApplyHead.user_apply_status==2?'审批中':''}}{{props.row.userApplyHead.user_apply_status==3?'已出库':''}}</span>
                    </template>
                  </el-table-column>
                  <el-table-column
                    label="操作"
  ...
  export default {
      ...
      methods: {
          ...
          notapprove(res){
                  ...
                  self.$prompt('请输入暂不出库的原因', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    inputPattern: /\S/,
                    inputErrorMessage: '请填写暂不出库的原因'
                  }).then(({ value }) => {
                    ...
                    self.$axios({
                        method: 'post',
                        url: '/store/stock/cannotConfirmOut',
                        data: params,
          ...
          approve(res){
                  ...
                  var params = new URLSearchParams();
                  var token = localStorage.getItem('auth');
                  self.$prompt('请输入领料人', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    inputPattern: /^[\u4E00-\u9FA5\uf900-\ufa2d·s]{2,20}$/,
                    inputErrorMessage: '请填写领料人真实姓名'
                  }).then(({ value }) => {
                    params.append('user_apply_head_id',res.user_apply_head_id); 
                    params.append('lingyong_user_true_name',value);    
                    self.$axios({
                        method: 'post',
