                ...
                <el-table
                    ...
                    <el-table-column
                      ...
                      <template slot-scope="props">
                        ...
                        size="mini"
                        type="warning"
                        @click="zhouqiClick(props.row)">确认已发放</el-button>
                      </template>
                    </el-table-column>
                </el-table>
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            zhouqiClick(res){
                ...
                var token = localStorage.getItem('auth');
                console.log(res)
                params.append('user_laobao_id',res.user_laobao_id);
                self.$confirm('是否确认发放?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    self.$axios({
                        method: 'post',
                        url: '/store/laobao/confirmFaFang',
                        data: params,
                        headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
                    }).then(function(data){
                        if(data.data.code==0){
            ...
            getDataList(){
                ...
                self.$axios({
                    method: 'post',
                    url: '/store/laobao/getThisMonthLaoBao',
                    data: params,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
                 }).then(function(data){
                    if(data.data.code==0){
