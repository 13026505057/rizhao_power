...
<!--         <div >
        ...
        <div id="container" style="width: 100%;height: 110%;  float: right;">
          <div class="numData">
            ...
            <div class="tableTitle">
              <el-button type="primary" @click="showAddNew">新增考试资料</el-button>
            </div>
          </div>
            <el-dialog title="新增资料" :visible.sync="dialogFormVisible">
              <el-form :model="form">
                <el-form-item label="资料名称:" label-width="120px">
                  <el-input v-model="form.datum_name" autocomplete="off" style="width:400px;float:left;"></el-input>
                </el-form-item>
                <el-form-item label="资料描述:" label-width="120px">
                  <el-input v-model="form.datum_desc" autocomplete="off" style="width:400px;float:left;"></el-input>
                </el-form-item>
                <el-upload
                  class="upload-demo"
                  ...
                  <div class="el-upload__tip" slot="tip">只能上传图片/视频/文档等类型文件，且不超过50Mb</div>
                </el-upload>
              </el-form>
            ...
            <el-dialog title="修改资料" :visible.sync="dialogFormVisible2">
              <el-form :model="form">
                <el-form-item label="资料名称:" label-width="120px">
                  <el-input v-model="form.datum_name" autocomplete="off" style="width:400px;"></el-input>
                </el-form-item>
                <el-form-item label="资料描述:" label-width="120px">
                  <el-input v-model="form.datum_desc" autocomplete="off" style="width:400px;"></el-input>
                </el-form-item>
                <el-form-item label="资料状态:" label-width="120px">
                  <el-select v-model="form.datum_status" placeholder="请选择状态">
                    <el-option
            ...
            <el-dialog title="地图选点组件" :visible.sync="mapVisible">
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="props">
                  <el-button size="mini" type="warning"  @click="downLoadFile(props.row.datum_url)">下载</el-button>
                </template>
              </el-table-column>
              ...
              <el-table-column
                ...
                >
                <template slot-scope="props">
                  <span :class="[props.row.datum_status==0?'colorRed':'']">{{props.row.datum_status==0?'未启用':'启用'}}</span>
                </template>
              </el-table-column>
...
<script>
  ...
  export default {
      ...
      mounted() {
          ...
          this.uploadUrl = this.$axios.defaults.baseURL+'stop/exam/datum/uploadFile';
      ...
      methods: {
          ...
          uploadSuccess(response, file, fileList){
            ...
            if(response.code==0){
              this.uploadUrlCount = response.data;
            }else{
              this.$message({
                  type: 'error',
          ...
          upDataLocalMapMsg(){
            if(localStorage.getItem('isLocalChoosed')==1){
              this.form.latitude = localStorage.getItem('localLat');
              this.form.longitude = localStorage.getItem('localLng');
              this.form.imac_location = localStorage.getItem('locationDes');
          ...
          handleEdit(data){
              ...
              self.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                ...
                type: 'warning'
                  ...
                  params.append('datum_id',data.datum_id);    
                  self.$axios({
                      method: 'post',
                      url: '/stop/exam/datum/del',
                      data: params,
          ...
          upData(data){
                ...
                var params = new URLSearchParams();
                var token = localStorage.getItem('auth');
                params.append('datum_id',self.form.datum_id);    
                params.append('datum_status',self.form.datum_status);
                params.append('datum_name',self.form.datum_name);
                params.append('datum_desc',self.form.datum_desc);
                self.$axios({
                    method: 'post',
                    url: '/stop/exam/datum/update',
                    data: params,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
                 }).then(function(data){
                    if(data.data.code==0){
          ...
          addNew(){
                ...
                var token = localStorage.getItem('auth');
                params.append('datum_desc',self.form.datum_desc);
                params.append('datum_url',self.uploadUrlCount);
                params.append('datum_name',self.form.datum_name);          
                self.$axios({
                    method: 'post',
                    url: '/stop/exam/datum/add',
                    data: params,
                    headers: {'Content-Type': 'application/x-www-form-urlencoded','kf-token':token},
                 }).then(function(data){
                    if(data.data.code==0){
